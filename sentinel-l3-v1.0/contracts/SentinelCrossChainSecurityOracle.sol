...
contract SentinelCrossChainSecurityOracle is LzApp {
  ...
  function shareSecurityData(
    ...
    bytes32 dataHash = keccak256(
      abi.encodePacked(dstChainId, threatLevel, anomalyCount, activeAlerts, block.timestamp)
    );
   ...
   */
  function _getLatestChainData(uint16 chainId) internal pure returns (SecurityData memory) {
    chainId; // silence unused
