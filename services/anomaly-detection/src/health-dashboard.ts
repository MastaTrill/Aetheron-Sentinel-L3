...
async function startDashboard() {
  ...
  monitor.on("metricsUpdate", (metrics) => {
    ...
    console.log(
      `   Error Rate: ${((metrics.failedRequests / Math.max(metrics.totalRequests, 1)) * 100).toFixed(1)}%`,
    );
    console.log(
      `   Avg Response Time: ${metrics.averageResponseTime.toFixed(0)}ms`,
    );
    console.log(
      `   Overall Health: ${getStatusIcon(monitor.getOverallHealth())} ${monitor.getOverallHealth()}`,
    );
