...
export class AnomalyDetector extends EventEmitter {
  ...
  private checkTVLSpike(): void {
    if (this.tvlHistory.length < 10) return; // Need sufficient history for statistical analysis
    ...
    if (recentHistory.length >= 2) {
      ...
      if (previous.currentTVL === 0n) return;

      const drop = Number(
        ((previous.currentTVL - current.currentTVL) * 10000n) /
          previous.currentTVL,
      );
  ...
  private calculateZScore(current: TVLData, history: TVLData[]): number {
    ...
    for (let i = 1; i < history.length; i++) {
      ...
      if (prev.currentTVL === 0n) continue;

      const change = Number(
        ((prev.currentTVL - curr.currentTVL) * 10000n) / prev.currentTVL,
      );
      changes.push(change);
    }
    ...
    if (secondLast.currentTVL === 0n) return 0;

    const currentChange = Number(
      ((secondLast.currentTVL - current.currentTVL) * 10000n) /
  ...
  private analyzePatterns(): void {
    ...
    const recentWithdrawals = this.withdrawalHistory.filter(
      (w) => now - w.timestamp < this.config.withdrawalWindow,
    );
    ...
    if (recentWithdrawals.length >= 5) {
      const withdrawalZScore =
        this.calculateWithdrawalZScore(recentWithdrawals);
    ...
    if (recentWithdrawals.length >= 3) {
      ...
      if (avgTVL > 0n) {
        for (const withdrawal of recentWithdrawals) {
          const percentage =
            Number((withdrawal.amount * 10000n) / avgTVL) / 100;
          const withdrawalZScore = this.calculateSingleWithdrawalZScore(
            withdrawal.amount,
            recentWithdrawals,
          );
  ...
  private checkWithdrawalSpike(withdrawals: WithdrawalEvent[]): void {
    if (withdrawals.length < 3) return;
  ...
  private calculateWithdrawalZScore(withdrawals: WithdrawalEvent[]): number {
    ...
    if (historicalPatterns.length < 5) return 0; // Need sufficient history
    ...
    const totalMean =
      totalMeans.reduce((a, b) => a + b, 0n) / BigInt(totalMeans.length);
    const countMean = countMeans.reduce((a, b) => a + b, 0) / countMeans.length;

    const totalVariance =
      totalMeans.reduce((sum, val) => {
        const diff = Number(val - totalMean);
        return sum + diff * diff;
      }, 0) / totalMeans.length;

    const countVariance =
      countMeans.reduce((sum, val) => {
    ...
    if (totalStdDev === 0 || countStdDev === 0) return 0;
  ...
  private calculateWithdrawalVelocityZScore(
    ...
    if (historicalVelocities.length < 3) return 0;

    const velocityMean =
      historicalVelocities.reduce((a, b) => a + b, 0) /
      historicalVelocities.length;
    const velocityVariance =
      historicalVelocities.reduce(
        (sum, val) => sum + Math.pow(val - velocityMean, 2),
        0,
      ) / historicalVelocities.length;
    const velocityStdDev = Math.sqrt(velocityVariance);

    if (velocityStdDev === 0) return 0;

    return (velocity - velocityMean) / velocityStdDev;
  }

  private getHistoricalWithdrawalPatterns(): Array<{
    total: bigint;
    count: number;
  }> {
  ...
  private getHistoricalWithdrawalVelocities(): number[] {
    // Historical withdrawal velocities (withdrawals per second)
    return [0.1, 0.15, 0.08, 0.12, 0.09, 0.11, 0.13];
  }

  private recordWithdrawal(event: WithdrawalEvent): void {
  ...
  getPerformanceMetrics() {
    ...
    const avgTVLFetchTime =
      this.metrics.tvlFetchCount > 0
        ? this.metrics.tvlFetchTotalTime / this.metrics.tvlFetchCount
        : 0;
    const avgDetectionTime =
      this.metrics.anomalyDetectionCount > 0
