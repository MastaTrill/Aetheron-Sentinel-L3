...
async function main() {
  ...
  console.log(
    `Checking bytecode for SentinelCore at: ${coreAddress} on network: ${hre.network.name}`
  );
  ...
  if (onChainBytecode === localBytecode) {
    ...
    console.log(
      'This is common if the contract uses immutable variables (which are baked into the bytecode at deploy time) or if the compiler versions differ slightly.'
    );
    console.log(
      `Local Length: ${localBytecode.length} | On-Chain Length: ${onChainBytecode.length}`
    );
  }
}
