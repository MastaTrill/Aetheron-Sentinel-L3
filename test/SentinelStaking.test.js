...
describe('SentinelStaking', function () {
  ...
  describe('Deployment', function () {
    it('reverts with zero staking token address', async function () {
      const SentinelStaking = await ethers.getContractFactory('SentinelStaking');
      await expect(
        SentinelStaking.deploy(ethers.ZeroAddress, await rewardToken.getAddress(), owner.address)
    ...
    it('reverts with zero reward token address', async function () {
      const SentinelStaking = await ethers.getContractFactory('SentinelStaking');
      await expect(
        SentinelStaking.deploy(await stakingToken.getAddress(), ethers.ZeroAddress, owner.address)
  ...
  describe('stake', function () {
    it('reverts when amount is 0', async function () {
      await expect(staking.connect(user).stake(0)).to.be.revertedWith('Cannot stake 0');
    });

    it('transfers tokens into the contract', async function () {
      const amount = ethers.parseEther('100');
      const contractBefore = await stakingToken.balanceOf(await staking.getAddress());
      await staking.connect(user).stake(amount);
      const contractAfter = await stakingToken.balanceOf(await staking.getAddress());
  ...
  describe('unstake', function () {
    ...
    it('reverts when unstaking more than staked', async function () {
      await expect(staking.connect(user).unstake(ethers.parseEther('1000'))).to.be.revertedWith(
        'Insufficient stake'
      );
    });

    it('reverts when lock period has not elapsed', async function () {
      await expect(staking.connect(user).unstake(ethers.parseEther('100'))).to.be.revertedWith(
        'Still locked'
      );
    });
