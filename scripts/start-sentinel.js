...
function loadEnv() {
    ...
    if (envConfig.error) {
        console.warn('⚠️ .env file not found or could not be loaded. Ensure it exists and is correctly formatted.');
    }
}
...
async function main() {
    try {
        ...
        for (let i = 1; i <= maxAttempts; i++) {
            ...
            unreachableContracts = addresses.filter((addr, index) => results[index] === '0x' || results[index] === '0x0');
            reachable = unreachableContracts.length === 0;

            if (reachable) break;

            if (i < maxAttempts) {
                console.log(`⏳ Some contracts not yet reachable: ${unreachableContracts.join(', ')}. Waiting 5s...`);
                await new Promise(resolve => setTimeout(resolve, 5000));
            }
        }

        if (!reachable) {
            throw new Error(`Timeout: The following contracts are not reachable on the network: ${unreachableContracts.join(', ')}`);
        }
