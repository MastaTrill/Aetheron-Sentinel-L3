...
async function propagateAlert(targetChainId, severity) {
  ...
  try {
    ...
    if (wallet.sendTransaction) {
      // Check if it's a CdpWalletProvider or similar signer
      ...
      // Fallback to ethers.Wallet as a signer for a Contract instance
      const bridgeContract = new ethers.Contract(bridgeAddress, bridgeInterface, wallet);
      tx = await bridgeContract.sendSecurityAlert(targetChainId, severity, {
