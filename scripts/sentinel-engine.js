...
class SentinelEngine {
  ...
  startHealthCheck() {
    ...
    app.get('/health', (req, res) => {
      const now = Date.now();
      const isStalled = now - this.lastHeartbeat > 300000; // 5 minutes

      const uptimeSeconds = (now - this.startTime) / 1000;
      const throughput = (this.metrics.processed / uptimeSeconds).toFixed(2);
      const avgLatency =
        this.metrics.processed > 0
          ? (this.metrics.totalLatency / this.metrics.processed).toFixed(2)
          : 0;
      ...
      res.status(isStalled ? 503 : 200).json({
        ...
        throughput_tx_s: parseFloat(throughput),
  ...
  async initializeWallet() {
    ...
    if (cdpKeyName && cdpKeyPrivate) {
      ...
      console.warn(
        '⚠️ Warning: Using raw private key from environment. Consider using a keystore.'
      );
    ...
    const address = this.walletProvider
      ? await this.walletProvider.getAddress()
  ...
  async start() {
    ...
    monitorContract.on('DataTracked', async (chainId, data, event) => {
      ...
      await this.processTransaction(
        { rawBytes: data, context: 'DATA_TRACKED' },
    ...
    monitorContract.on(
      'TransactionObserved',
      async (chainId, sender, target, value, data, context, timestamp, event) => {
        this.lastHeartbeat = Date.now();
        console.log(
          `[Chain ${chainId}] Observed transaction from ${sender} to ${target}. Analyzing...`
        );
        await this.processTransaction(
          {
            sender,
  ...
  async mitigateThreat(txHash, analysis) {
    ...
    const interceptorInterface = new ethers.Interface([
      'function triggerEmergencyPause(bytes32 reasonId) external',
    ]);
    ...
    try {
      ...
      console.log(
        `✅ Emergency pause triggered. Transaction identifier: ${txResponse.hash || txResponse}`
      );
    } catch (error) {
