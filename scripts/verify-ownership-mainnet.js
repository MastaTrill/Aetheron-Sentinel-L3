    ...
    try {
      ...
      const status =
        currentOwner.toLowerCase() === expectedOwner.toLowerCase()
          ? '✅'
          : `❌ WRONG OWNER (Expected ${isTreasuryRouted ? 'Treasury' : 'Admin'})`;
      ...
      try {
        ...
        if (pending !== ethers.ZeroAddress) {
          ...
          console.log(
            `  ⚠️  PENDING TRANSFER: ${pending} (${isTreasury ? 'Treasury' : 'Unknown'})`
          );
    ...
    try {
      ...
      const tOk =
        threshold === EXPECTED_THRESHOLD
          ? '✅'
          : `❌ (Expected ${ethers.formatUnits(EXPECTED_THRESHOLD, 18)})`;
      const tokenOk =
        govToken.toLowerCase() === targetToken.toLowerCase()
          ? '✅'
