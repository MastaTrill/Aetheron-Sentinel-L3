...
task(
  'simulate-lattice-failure',
  'Tests SentinelCoreLoop resilience against 25% calibration failures'
  ...
  if (finalShrunkWindow === minWindow) {
    console.log(
      `✅ PASS: Window correctly floor-capped at s_minAnomalyWindow (${finalShrunkWindow}s)`
    );
  } else {
    console.log(
      `❌ FAIL: Window shrank below floor or logic error. Got ${finalShrunkWindow}s, expected ${minWindow}s`
    );
  }

  // Verify s_windowStartTimestamp was updated during high-threat calibration
  if (
    finalWindowStartTimestamp > initialWindowStartTimestamp &&
    finalWindowStartTimestamp <= latestBlock.timestamp
  ) {
    console.log(
      `✅ PASS: s_windowStartTimestamp updated from ${initialWindowStartTimestamp} to ${finalWindowStartTimestamp} (latest block.timestamp: ${latestBlock.timestamp})`
    );
  } else {
    console.log(
      `❌ FAIL: s_windowStartTimestamp not updated correctly. Initial: ${initialWindowStartTimestamp}, Final: ${finalWindowStartTimestamp}, Latest block.timestamp: ${latestBlock.timestamp}`
    );
  }
  ...
  if (stableWindow !== maxWindow) {
    console.log(
      `❌ FAIL: Window did not settle at maxWindow. Current: ${stableWindow}s, Expected: ${maxWindow}s`
    );
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (currentStableWindow !== maxWindow) {
      console.log(
        `❌ FAIL: Window changed from ${maxWindow}s to ${currentStableWindow}s after recovery.`
      );
      throw new Error('Window did not remain stable at maxWindow.');
    }
  }
  console.log(
    `✅ PASS: Window remained stable at s_maxAnomalyWindow (${maxWindow}s) after multiple triggers.`
  );
