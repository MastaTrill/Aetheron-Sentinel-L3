...
contract SocialRecovery is AccessControl, Pausable {
     ...
     */
    function updateGuardianWeight(address guardian, uint256 newWeight) 
        external 
        onlyOwner 
    {
        if (!isGuardian[guardian]) revert NotGuardian(guardian);
    ...
    function confirmRecovery(bytes32 requestId) external {
        ...
        emit RecoveryConfirmed(requestId, msg.sender, guardianWeight[msg.sender]);
        
        // Check if threshold reached
        uint256 totalWeight = _getTotalGuardianWeight();
        if (request.confirmations * 100 / totalWeight >= uint256(recoveryThreshold) * 100 / guardians.length) {
    ...
    function executeRecovery(bytes32 requestId) external {
        ...
        // Check window
        if (block.timestamp > request.initiateTime + RECOVERY_DELAY + RECOVERY_WINDOW) {
            revert RecoveryExpired(requestId);
        }
        
        // Check threshold
        uint256 totalWeight = _getTotalGuardianWeight();
        if (request.confirmations * 100 / totalWeight < recoveryThreshold * 100 / guardians.length) {
    ...
    function cancelRecovery(bytes32 requestId) external {
        ...
        bool isGuardianVote = isGuardian[msg.sender] && request.confirmedBy[msg.sender];
    ...
    function getRecoveryInfo(bytes32 requestId) external view returns (
        address lostAccount,
        address newOwner,
        uint256 initiateTime,
        uint256 confirmations,
    ...
    function hasConfirmed(bytes32 requestId, address guardian) external view returns (bool) {
