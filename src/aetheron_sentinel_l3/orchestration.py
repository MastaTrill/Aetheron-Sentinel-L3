...
class PauseResumeOrchestrator:
    ...
    def handle_bmnr_alert(self, alert: BmnrAlert) -> OrchestrationDecision:
        ...
        if correlated.action == "pause":
            ...
            return OrchestrationDecision("pause_bridge", "bmnr_high_severity", correlated, execution)
        if correlated.action == "resume":
            ...
            return OrchestrationDecision("resume_bridge", "bmnr_resume_signal", correlated, execution)
    ...
    def handle_sentinel_decision(
        ...
        *,
        ...
        if action == "BLOCK":
            ...
            return OrchestrationDecision("pause_bridge", "sentinel_block", correlated, execution)

        if action == "ALLOW" and correlated is not None and correlated.action == "resume":
            execution = self.executor.apply_controls(("resume_bridge",))
            self._record("sentinel_resume", event_id, risk_score, correlated, execution)
            return OrchestrationDecision("resume_bridge", "sentinel_allow_after_resume", correlated, execution)

        self._record("sentinel_monitor", event_id, risk_score, correlated, None)
        return OrchestrationDecision("monitor", "sentinel_non_blocking", correlated, None)
    ...
    def _record(
        ...
        if correlated is not None:
            reasons.extend([correlated.correlation_key, correlated.source, correlated.action])
