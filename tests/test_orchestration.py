...
class TestPauseResumeOrchestrator(unittest.TestCase):
    def make_orchestrator(self, *, fail_controls: set[str] | None = None):
        state = {"paused": False}
        ...
        orchestrator = PauseResumeOrchestrator(executor=executor, sink=sink, correlation_engine=engine)
    ...
    def test_failed_pause_execution_is_recorded(self) -> None:
        orchestrator, sink, state = self.make_orchestrator(fail_controls={"pause_bridge"})
        alert = BmnrAlert(
            id="bmnr-5",
            type="anomaly",
            severity="critical",
