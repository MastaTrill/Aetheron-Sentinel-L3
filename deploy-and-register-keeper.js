...
async function main() {
  ...
  if (!addrs) {
    throw new Error(
      `Unsupported network chainId: ${chainId}. Add Chainlink addresses for this network.`
    );
  }
  ...
  if (!fs.existsSync(DEPLOYED_ADDRESSES_PATH)) {
    throw new Error(
      `DEPLOYED_ADDRESSES.json not found at ${DEPLOYED_ADDRESSES_PATH}. Run orchestrator first.`
    );
  }
  const deployedAddresses = JSON.parse(fs.readFileSync(DEPLOYED_ADDRESSES_PATH, 'utf8'));
  const CORE_ADDRESS =
    deployedAddresses.SentinelCore || deployedAddresses.contracts?.SentinelCore?.address;
  if (!CORE_ADDRESS) {
  ...
  const log = receipt.logs.find(
    x => x.topics[0] === registrarInterface.getEventTopic('UpkeepRegistered')
  );
  ...
  if (log) {
    ...
    console.log(
      'Warning: UpkeepRegistered event not found in logs. You may need to fetch the Forwarder manually from the dashboard.'
    );
  }
  ...
  if (forwarderAddress !== ethers.ZeroAddress) {
    ...
    console.log(
      '⚠️ Warning: Forwarder address is ZeroAddress. Manual setForwarder call may be required.'
    );
  }

  console.log('\nNext Steps:');
  console.log(
    `1. Run 'node handover-to-keeper.cjs' to grant keeper permissions (ownership of SentinelCore).`
  );
}
